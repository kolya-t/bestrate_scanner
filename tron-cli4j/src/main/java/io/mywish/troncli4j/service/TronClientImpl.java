...
import io.mywish.troncli4j.model.FoTransaction;
...
public class TronClientImpl implements TronClient {
    ...
    private BlockResponse parseBlock(JsonNode blockNode) throws Exception {
        BlockResponse response = objectMapper.treeToValue(blockNode, BlockResponse.class);
        response.getTransactions().forEach(this::fillFeeTransaction);
        return response;
    }
    ...
    private void fillFeeTransaction(Transaction transaction) {
        transaction.setFee(0L);
        Long fee = null;
        try {
            fee = objectMapper.treeToValue(
                    doRequest(fullNodeRpc + "/wallet/gettransactioninfobyid?value=" + transaction.getTxId()), FoTransaction.class).getFee();
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (fee != null) {
